...
void NRF24L01_init(void)
{
	...
	*/
	...
	if( LED.status != AlternateFlash && MPU_Err !=1){//方式失联后，频繁重设灯状态，导致闪烁不连续
				bLED_H();aLED_H(); //右红，左红 灭
				fLED_L();hLED_L(); //右蓝，左蓝 亮
				LED.status = AlternateFlash; //开始交替闪烁，表示尚未连接遥控器
	} 
  ...
  do
	{ 
		GetLockCode(); //获取ST_CpuID
		_CH = ST_CpuID % 0x7E;      //芯片ID用来做通讯对频通道
		ANO_NRF_Init(MODEL_RX2,_CH);
		NRF_Err=1;
	}while(NRF24L01_Check() == FAILED);  //spi通讯成功才会继续
...
void ANO_NRF_Check_Event(void)
{
	...
	if(sta & (1<<RX_DR))																	//判断是否收到数据
	{
		...
		if(rx_len<33)
		{
			...
			Nrf_Erro = 0;  //表示收到数据，在随后的NRF_Connect()会变为1
		}
		else 
		{
...
//4ms一次， remote.c调用； 
//Nrf_Erro累加到250表示已经1秒没有收到遥控数据
u8 NRF_Connect(void)//
{
	static u8 Connect_flag;
	
	Nrf_Erro ++; //Nrf_Erro为0表示收到数据 ( 收到数据后，由函数ANO_NRF_Check_Event()重设Nrf_Erro )，自增为变为1
	if(Nrf_Erro==1) //自增后 1 表示收到数据
	{
		ANO_DT_Data_Receive_Anl(NRF24L01_2_RXDATA,NRF24L01_2_RXDATA[3]+5);//解析2.4G数据
		NRF_SSI_CNT++;
		
		//收到数据后，如果LED灯状态还是交替闪烁 AlternateFlash或者AllFlashLight
		//那么改为同步闪烁 SimpleFlash
		//有时，rf初始化后，仍然为AllFlashLight
		if((LED.status==AllFlashLight || LED.status==AlternateFlash) && Remote.pitch!=0){ //刚上电时，没有连接遥控也会收到乱码数据
					bLED_H();aLED_H();fLED_H();hLED_H(); //同步状态
					LED.status = SimpleFlash; //开始同步闪烁，表示已经连上遥控器
		} 
	...
	if(Nrf_Erro>=500)  //4*500=2000ms，，，在remote.c的 RC_Analy()  3秒后会重启rf模块，指示灯会变成交替闪烁
	{
		//Nrf_Erro = 1;//=1表示收到数据了，没有信号也尝试一下,设置为1不也会进去收数据的
		Nrf_Erro = 2; //=1表示收到数据了，这里需要复位成2
		Connect_flag = 0;
	}
	return Connect_flag;
}
